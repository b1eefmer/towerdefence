...
public static class SaveSystem
{
    ...
    private static SaveSystemRunner runner;
    ...
    private static void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        EnsureRunner();
        runner.StartCoroutine(ApplyLoadedDataDeferred(pendingLoadData));
    }

    private static IEnumerator ApplyLoadedDataDeferred(GameSaveData saveData)
    {
        yield return null;

        ApplyLoadedData(saveData);

        if (pendingLoadData == saveData)
        {
    ...
    private static void EnsureRunner()
    {
        ...
        GameObject runnerObject = GameObject.Find("SaveSystemRunner");
        if (runnerObject == null)
        {
            runnerObject = new GameObject("SaveSystemRunner");
            UnityEngine.Object.DontDestroyOnLoad(runnerObject);
        }

        runner = runnerObject.GetComponent<SaveSystemRunner>();
        if (runner == null)
        {
            runner = runnerObject.AddComponent<SaveSystemRunner>();
        }
    }
    ...
    private sealed class SaveSystemRunner : MonoBehaviour
    {
    }
}
